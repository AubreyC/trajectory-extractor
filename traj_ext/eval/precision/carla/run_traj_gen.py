...
# Strongly instpired from: https://github.com/carla-simulator/carla/blob/0.8.4/PythonClient/manual_control.py
#
...
sys.path.append(os.path.join(FILE_PATH, "carla/PythonClient"))  # To find local version
...
class CarlaGame(object):
    def __init__(self, carla_client, args):
        ...
        dir_csv = "carla_traj_csv/" + now.strftime("%Y-%m-%d_%H:%M:%S");
        dir_csv = os.path.join(FILE_PATH, dir_csv)
    ...
    def _on_loop(self):
        ...
        # Create entry for the player agent (ego agent)
        data = {'id': -1, \
                 'timestamp_ms': measurements.game_timestamp,\
                 'agent_type': 'vehicle',\
                 'x': measurements.player_measurements.transform.location.x,\
                 'y': measurements.player_measurements.transform.location.y,\
                 'vx': np.cos(np.deg2rad(measurements.player_measurements.transform.rotation.yaw))*(measurements.player_measurements.forward_speed),\
                 'vy': np.sin(np.deg2rad(measurements.player_measurements.transform.rotation.yaw))*(measurements.player_measurements.forward_speed),\
                 'psi_rad':  np.deg2rad(measurements.player_measurements.transform.rotation.yaw)}
        agent_results.append(data);

        # Create entries for the other players
        for agent in measurements.non_player_agents:

            # Only log if it's a vehicle
            if agent.HasField('vehicle'):

                data = {'id': agent.id, \
                         'timestamp_ms': measurements.game_timestamp,\
                         'agent_type': 'vehicle',\
                         'x': agent.vehicle.transform.location.x,\
                         'y': agent.vehicle.transform.location.y,\
                         'vx': np.cos(np.deg2rad(agent.vehicle.transform.rotation.yaw))*(agent.vehicle.forward_speed),\
                         'vy': np.sin(np.deg2rad(agent.vehicle.transform.rotation.yaw))*(agent.vehicle.forward_speed),\
                         'psi_rad':  np.deg2rad(agent.vehicle.transform.rotation.yaw)}
                # Add data entry
                agent_results.append(data);


        # time_ms_str = str(measurements.game_timestamp).zfill(10);
        index_str =  str(self._timer.step -1).zfill(8);
        csv_name = 'carla_traj_truth_' + index_str + '.csv';
        trajutil.write_traj_csv(os.path.join(self.dir_csv, csv_name), agent_results);
