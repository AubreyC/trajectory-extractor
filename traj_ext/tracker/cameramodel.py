...
if __name__ == '__main__':
    # Define matrix:
    cam_matrix = np.matrix([[1920,   0, 1920/2],\
                            [  0, 1920, 1080/2],\
                            [  0,   0,      1]]);

    rot_CF_F = eulerAnglesToRotationMatrix([np.deg2rad(60), 0 , 0]);

    pixel_xy = np.array([1920/2, 300]);
    trans_CF_F = rot_CF_F.dot(np.array([0,0,10]));
    pos_F = projection_ground(cam_matrix, rot_CF_F, trans_CF_F, 0, pixel_xy);
    print(pos_F)
