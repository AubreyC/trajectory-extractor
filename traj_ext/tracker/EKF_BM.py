...
class EKF_BM_track(EKF_track):
    ...
    def get_A_model(self, x_current):
        ...
        theta = self.clamp(x_current[4,0], -np.pi, np.pi);
        ...
        A = np.array([[ 0,  0,                    np.cos(phi),  -v*np.sin(phi) -phi_dot*lr*np.cos(phi),      -lr*np.sin(phi)*float(v)/float(lr+lf)*(1/float(pow(np.cos(theta),2)))],\
                       [ 0,  0,                   np.sin(phi),   v*np.cos(phi) -phi_dot*lr*np.sin(phi),       lr*np.cos(phi)*float(v)/float(lr+lf)*(1/float(pow(np.cos(theta),2)))],\
                       [ 0,  0,                              0,                                         0,                                                                              0],\
                       [ 0,  0, np.tan(theta)/(float(lr+lf)),                                         0,                        float(v)/float(lr+lf)*(1/float(pow(np.cos(theta),2)))],\
                       [ 0,  0,                              0,                                         0,                                                                              0]]);
    ...
    def propagate_state(self, x_current, delta_s):
        ...
        phi_dot = float(v)/(lr+lf)*np.tan(theta);
        x_dot = np.array([[v*np.cos(phi) - phi_dot*lr*np.sin(phi),\
                            v*np.sin(phi) + phi_dot*lr*np.cos(phi), \
                            0,\
                            phi_dot, \
                            0]]);
